...
EPISODES        = 500
STEPS_PER_EP    = 3600
TOTAL_TIMESTEPS = EPISODES * STEPS_PER_EP  # 1,800,000


# ── Custom callback: logs per-episode stats to TensorBoard ─────────────────
class EpisodeLoggerCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        if self.locals["dones"][0]:
            ...
            print(
                f"Episode {self.episode_count:>3} | "
...
def train():
    print("=" * 60)
    print("  Traffic Signal Control - PPO Training")
    ...
    # ── Create environment ─────────────────────────────────────────────────
    env = TrafficEnv(use_gui=False, episode_length=STEPS_PER_EP)

    # ── Create PPO model with thesis hyperparameters ───────────────────────
    model = PPO(
        policy          = "MlpPolicy",
        env             = env,
        learning_rate   = 3e-4,
    ...
    checkpoint_callback = CheckpointCallback(
        save_freq  = STEPS_PER_EP * 10,   # save every 10 episodes
